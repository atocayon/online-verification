<form>
  <br>
  <div class="column">
    <div class="">
        <label>Certificate Number</label>
    </div>

    <div class="form-input-spacing">
      <input type="text" name="cert" class="form-input-name" placeholder="Type here..." id="byCert_certnum" title="Type your Certificate Number here" required>
    </div>

    <div class="">
      <label>First Name</label>
    </div>
    <div class="form-input-spacing">
      <input type="text" name="fname" id="byCert_fname" autocomplete="off" autofocus class="form-input-name" placeholder="Type here..." title="Type your First Name here"  required>
    </div>

    <div class="">
        <label>Last Name</label>
    </div>
    <div class="form-input-spacing">
      <input type="text" name="lname" id="byCert_lname" class="form-input-name" autocomplete="off" autofocus placeholder="Type here..." title="Type your Last Name here" required>
    </div>

    <div class="">
      <label>Birthday</label>
    </div>

    <div class="form-input-spacing">
      <input type="text" name="bday" id="byCert_bday" class="form-input-date" autocomplete="off" title="Put your Birthday here" autofocus required>
    </div>


    <div class="">
      <center>
        <button type="submit" class="submit-button" id="byCert_submit" title="Click to Verify">Verify</button>
      </center>
    </div>

    <div class="">
      <center>
        <button id="searhAgain_byCert" class="tryAgain-button" title="Click to Clear Searches">Clear Searches</button>
      </center>
    </div>

    <div class="form-input-spacing">
      <center>
        <button id="mobile-byCert_submit" class="mobile-button" title="Click to Verify">Verify</button>
      </center>
    </div>
  </div>

</form>
